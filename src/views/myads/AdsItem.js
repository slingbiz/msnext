  ...
  return (
    <Paper sx={{ width: 'full', p: 2, display: 'flex', flexDirection: 'column', my: 2 }}>
      ...
      <Box sx={{ display: 'flex', justifyContent: 'space-between', flexDirection: { xs: 'column', lg: 'row' } }}>
        <Box sx={{ display: 'flex' }}>
          <Typography variant='subtitle2' sx={{ fontSize: 15 }}>
            Posted on <span style={{ color: 'black', fontWeight: 600 }}>{moment(listing?.added_on).toString()}</span>
          </Typography>
          <Divider orientation='vertical' sx={{ mx: 2 }} flexItem />
          <Typography variant='subtitle2' sx={{ fontSize: 15 }}>
            Updated on <span style={{ color: 'black', fontWeight: 600 }}>{moment(listing?.updated_on).toString()}</span>
          </Typography>
        </Box>
